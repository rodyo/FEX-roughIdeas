...
% 
function varargout = ellipse(N, a,b, C, u,v)
    
    if nargin > 6 || nargout > 2
        error('ellipse:nargerror',...
            'ELLIPSE accepts maximum 4 input arguments, and 2 output aguments.');
    end
    ...
    % Basis vectors must be 2D vectors
    if ~isnumeric(u) || ~isvector(u) || numel(u) ~= 2 || ...
       ~isnumeric(v) || ~isvector(v) || numel(v) ~= 2
        error('ellipse:nonvector_basevectors',...
            'Input arguments ''u'' and ''v'' must be 2D vectors.');
    end
